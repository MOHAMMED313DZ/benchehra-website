    ...
    return (
      <div className="max-w-2xl mx-auto bg-card rounded-xl p-6 card-shadow">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-bold text-lg">{isNew ? (lang === "ar" ? "إضافة إعلان" : "Add Announcement") : (lang === "ar" ? "تعديل الإعلان" : "Edit")}</h2>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <p className="text-muted-foreground text-sm">{items.length} {lang === "ar" ? "إعلان" : "announcements"}</p>
        <Button type="button" onClick={() => { setEditing({ title_ar: "", title_en: "", content_ar: "", content_en: "" }); setIsNew(true); }}>
            ...
            <TableBody>
              {items.map((item) => (
                <TableRow key={item.announcement_id}>
                  ...
                  <TableCell dir="ltr" className="text-sm text-muted-foreground">{item.publish_date?.split("T")[0]}</TableCell>
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={() => remove(item.announcement_id)} className="p-1 hover:bg-destructive/10 rounded text-destructive transition-colors">
              ...
              {items.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="text-center py-10 text-muted-foreground opacity-50">
